...
namespace cpprefl
{
	...
	public:
		...
		template <typename T>
		T* GetFieldValueUnsafe(void* classObject, const char* fieldName)const;

		// Returns the value of a field in memory.
		// If the templated type does not match the field type, nullptr will be returned.
		template <typename T>
		T* GetFieldValueSafe(void* classObject, const char* fieldName)const;
	};

	template <typename T>
	T* ClassInfo::GetFieldValueUnsafe(void* classObject, const char* fieldName) const
	{
	...
	template <typename T>
	T* ClassInfo::GetFieldValueSafe(void* classObject, const char* fieldName) const
	{
		const FieldInfo* fieldInfo = GetField(fieldName);
